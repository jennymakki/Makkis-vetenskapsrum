  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0f0f0]">
      ...
      <main className="flex-1 p-8 max-w-6xl mx-auto">
        {/* Header / Intro */}
        <section className="mb-12 text-center bg-[#EDE623] text-[#1A1A1A] p-8 rounded-xl shadow-lg max-w-3xl mx-auto">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {unitsWithFiles.map((unit: Unit) => (
              ...
              >
                <h3 className="text-xl font-semibold text-[#EDE623]">
                  {unit.name}
                </h3>
                    ...
                    {unit.files.map((file: File) => (
                      <li key={file._id}>
                        <a
                          href={file.url}
                          download
                          className="block bg-[#EDE623] text-[#1A1A1A] px-4 py-2 rounded-lg text-sm font-medium text-center
                                     hover:bg-[#d6d111] hover:scale-105 transition transform duration-200"
                        >
                          {file.name}
                        </a>
                      </li>
